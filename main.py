...
scheduler.add_job(
    ...
    trigger='cron',
    hour=11,  # Runs daily at 23 PM or night 11 PM
    minute=5,  # Run exactly at the start of the hour
...
@app.post("/inprogress_start")
def inprogress_start(service_id:Annotated[int,Form()],type_of_activity:Annotated[str,Form()],no_of_items:Annotated[str,Form()],db:Session=Depends(get_db)):
     return crud.inprogress_start(db,service_id,type_of_activity,no_of_items)

@app.post("/inprogress_end/")
def inprogress_end(
    user_id: Annotated[int, Form()],
    service_id: Annotated[int, Form()],
    db: Session = Depends(get_db)
):
    return crud.inprogress_end(db, service_id, user_id)
...
def calculate_total_time(
   picked_date:Annotated[str,Form()],to_date:Annotated[str,Form()],db: Session = Depends(get_db)
    ...
    return total_time 
# -----------------------
# @app.post("/fetch-total-time/")
# def fetch_total_time(user_id: int, service_id: int, db: Session = Depends(get_db)):
#     fetch_total_time = crud.calculate_and_store_total_times(db, user_id, service_id)  # Correct the order here
#     return "success"
...
#     response_data = {
#         "message": "Success" if total_time_record else "No records found",
#         "user_id": user_id,
#         "service_id": service_id,
#         "date": total_time_record.date.isoformat() if total_time_record else "N/A",
#         "total_inprogress_time": total_time_record.total_inprogress_time or DEFAULT_TIME,
#         "total_hold_time": total_time_record.total_hold_time or DEFAULT_TIME,
#         "total_break_time": total_time_record.total_break_time or DEFAULT_TIME,
#         "total_meeting_time": total_time_record.total_meeting_time or DEFAULT_TIME,
#         "total_call_time": total_time_record.total_call_time or DEFAULT_TIME,
#         "total_ideal_time": total_time_record.total_ideal_time or DEFAULT_TIME,
#     }

#     return schemas.FetchTotalTimeResponse(**response_data)
# -----------------------
@app.post("/update-total-time/")
async def update_total_time(user_id: int, service_id: int, target_date: str, db: Session = Depends(get_db)):

    crud.fetch_total_time(db, user_id, service_id, target_date)
    return {
